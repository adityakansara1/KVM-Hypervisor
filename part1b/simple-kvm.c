...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	uint64_t memval = 0;
	uint32_t exit_count = 0, exit_io_in_count = 0, exit_io_out_count = 0, gva = -1, numsExitByTypeAddr = 0;
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			{ // HC_numExits
				exit_io_in_count++;
				*(uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset) = exit_count;
				continue;
			}
			...
			{ // HC_printStr
				exit_io_out_count++;
				char *p = (char *)vcpu->kvm_run;
				int offset = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				unsigned long hva = (unsigned long)(vm->mem + offset);
				printf("%s", (char *)hva);
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xE5)
			{
				...
				uint8_t flag = *(uint8_t *)(p + vcpu->kvm_run->io.data_offset);
				if (flag == 1)
				{
					numsExitByTypeAddr = 0x12000;
				}
				else
				{
					numsExitByTypeAddr = 0x13000;
				}
				continue;
			}
			...
			{ // HC_numExitsByType
				exit_io_in_count++;
				unsigned long hva = (unsigned long)vm->mem + numsExitByTypeAddr;
				sprintf((char *)hva, "IO in: %u\nIO out: %u\n", exit_io_in_count, exit_io_out_count);
				*(uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset) = numsExitByTypeAddr;
				continue;
			}
			...
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xE4)
			{
				exit_io_in_count++;
				char *p = (char *)vcpu->kvm_run;
				...
				if (tr.valid)
				{
					unsigned long hva = (unsigned long)(vm->mem + tr.physical_address);
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = hva;
				}
				else
				{
